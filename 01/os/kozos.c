...
typedef struct _kz_msgbox{
    ...
    /*
     * H8は16ビットCPUなので32ビット整数に対しての乗算命令が無い。よって
     * 構造体のサイズが2の累乗になっていないと、構造体の配列のインデックス計算で
     * 乗算が使われて「__mulsi3が無い」などのリンクエラーになる場合がある。
     * (2の累乗ならばシフト演算が利用されるので問題は出ない)
...
static int thread_exit(void){
     ...
     * このため、スレッドを頻繁に生成消去するようなことは現状できない
     */
...
static int thread_send(kz_msgbox_id_t id, int size, char *p){
    ...
    putcurrent();
    sendmsg(mboxp, current, size, p); /* メッセージを送信する*/
...
static kz_thread_id_t thread_recv(kz_msgbox_id_t id, int *sizep, char **pp){
    ...
    mboxp->receiver = current; /* 受信待ちスレッドの設定 */
    
    if(mboxp->head == NULL){
        /* メッセージボックスにメッセージが無いのでスレッドをスリープさせる
         * (システムコールがブロックする)
         */
        return -1;
    }
...
static int setintr(softvec_type_t type, kz_handler_t handler){
    ...
    handlers[type] = handler; return 0;
}
...
void kz_start(kz_func_t func, char *name, int priority, int stacksize, int argc, char* argv[]){
    /*
     * 以降で呼び出すスレッド関連のライブラリ関数の内部でcurrentを見ている
     * 場合があるので、currentをNULLに初期化しておく
     */
    ...
    /* システムコール発行不可なので、直接関数を呼び出してスレッドを生成する */
